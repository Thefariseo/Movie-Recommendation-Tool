...
// space expects them to love most, each with their best film for the member.
async function newDirectors(ctx, exclude) {
  if (!ctx.member) return [];
  const known = new Set([...(ctx.evidence?.directors?.keys() || [])].map(Number));
  // Eighteen a day, a different handful each day, so the row stays fresh and cheap.
  ...
  const ranked = pool.map((d, i) => {
    ...
    return { d, best: top[0]?.m, score: top.length ? top.reduce((s, x) => s + x.f, 0) / top.length : -Infinity };
  }).filter((x) => x.best).sort((a, b) => b.score - a.score).slice(0, 10);
  return ranked.map(({ d, best }) => ({ ...best, _caption: d.name }));
}
