...
const KUROSAWA = [5765, 'Akira Kurosawa'];
const library = [
...
test('what the critic learned opens threads of its own, without repeating one the ratings already show', () => {
  const rules = [
    { kind: 'person', id: 5765, name: 'Akira Kurosawa', stance: 'love' },
  ...
  assert.equal(threads.filter(t => t.id === 'director:5765').length, 1);
